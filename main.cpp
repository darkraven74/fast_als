...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (sarg == "--likes-format") {
            i++;
            likes_format = atoi(argv[i]);
        }
        else if (sarg == "--als-alfa") {
            i++;
            als_alfa = atof(argv[i]);
        }
        else if (sarg == "--als-error") {
            i++;
    ...
    fast_als als_alg(in, features_size, als_alfa, 30, csamples, likes_format, samples_for_calc_error_users,
